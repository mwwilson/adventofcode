...
class dir:
    ...
    def findCandidate(self, size, candidate):
        for key in self.subdirs:
            subCandidate = self.subdirs[key].findCandidate(size, candidate)
            if subCandidate >= size and subCandidate < candidate:
                candidate = subCandidate
        if self.size >= size and self.size < candidate:
            candidate = self.size
        return candidate
...
spaceAvailable = 70000000 - root.size
spaceNeeded = 30000000 - spaceAvailable

print(root.findCandidate(spaceNeeded, root.size))